...
export interface UserProfile {
    ...
    username: string;
    displayName: string;
    profileImage?: string;  // Make profileImage optional to match API response
    bio?: string;
    recipesCount: number;
...
export default function UserProfileClient({ userId }: UserProfileClientProps) {
    ...
    useEffect(() => {
        const fetchProfileData = async () => {
            ...
            try {
                // If userId is provided, fetch that user's public profile using the new endpoint
                if (userId && userId !== user?.uid) {
                    ...
                    setUserProfile(response.userData);
                    ...
                    if (user) {
                        ...
                        // For the current user, we still use the existing endpoint to get their recipes
                        // as it may include private/draft recipes not available in the public endpoint
                        const response = await apiFetch<{
                            success: boolean;
                            recipes?: Recipe[];
                            error?: string;
